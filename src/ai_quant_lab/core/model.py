...
class VersionedRef:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.object_id, GovernedId) or not isinstance(self.version, ObjectVersion):
...
class StateAssignment:
    ...
    def __post_init__(self) -> None:
        if not isinstance(self.axis, StateAxis) or not isinstance(self.value, str) or not self.value:
