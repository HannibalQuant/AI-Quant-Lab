...
PRECONDITIONS = (
    ...
    _ip(4, "Canonical serialization/ID/hash/equivalence", "AUD-01,DAT-05", "artifacts,data", "ART + DAT", "Approve representation scope", P.PARTIALLY_RESOLVED, "No unsupported lock claim", "", "T04,T36", "PARTIAL: dataset artifacts plus real-CSV source declarations/admission records persist and reconstruct with exact canonical identity; broader representation authority remains open"),
    _ip(5, "Time/clock/timezone/ordering rules", "DAT-03,ORC-02", "DAT,STA,commands", "STA + DAT", "Approve temporal specification", P.PARTIALLY_RESOLVED, "Hold ambiguity", "", "T16,T35", "PARTIAL: controlled CSV admission requires explicit UTC bar and source-availability semantics and never substitutes ingestion time; vendor late-data/calendar/clock trust policy remains open"),
    ...
    _ip(11, "Data quality/reconciliation semantics", "DAT-01,DAT-02,DAT-03,DAT-04,DAT-05", "DAT", "DAT + GOV", "Approve source contracts", P.PARTIALLY_RESOLVED, "Quarantine unknown/conflict", "IP-04,IP-05", "T15,T16,T32", "PARTIAL: declared source contracts drive bounded schema/order/duplicate/OHLC/gap admission checks; ADMITTED remains technical and grants no universal provider quality acceptance"),
    _ip(12, "Retention/privacy/licensing/legal policy", "AUD-01,AUD-02,AUD-03,DAT-01", "AUD,DAT,ledgers", "GOV + Legal + Data", "Approve policy", P.OPEN, "Preserve and restrict", "", "T25,T30", "OPEN: permission, license, retention, deletion and redistribution declarations are recorded fail-closed; no legal verification, policy authority, recovery or deletion workflow exists"),
    _ip(13, "Trust-boundary threat model/adapters", "IAM-01,HUM-01,ORC-01", "adapters,MACP,SMI", "Security + IAM", "Approve threat model", P.OPEN, "Adapters disabled", "IP-01,IP-02", "T22,T37", "OPEN: one explicit local file-snapshot boundary rejects path escape, symlinks, unsafe encoding and resource abuse; broader adapter threat model and authenticated operator authority remain unresolved"),
    _ip(14, "Implementation governance/change/release", "GOV-01,GOV-02,AUD-03", "Phase 3", "GOV + QA + AUD", "Approve wave gates", P.PARTIALLY_RESOLVED, "No wave promotion", "", "T30,T31,T38", "PARTIAL: real-CSV golden, deterministic admission/reload, provenance/lineage and hostile-input tests extend CI evidence; production release criteria remain open"),
)
