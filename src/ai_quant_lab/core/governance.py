...
PRECONDITIONS = (
    ...
    _ip(2, "Integrity/signature/tamper-evidence model", "AUD-01,AUD-02,AUD-03", "ART,EVI,AUD", "GOV + Security", "Approve integrity/signature model", P.PARTIALLY_RESOLVED, "Unverifiable is inadmissible", "IP-04", "T04,T05,T25", "PARTIAL: typed canonical bytes, integrity/tamper checks; signatures/trust roots open"),
    ...
    _ip(4, "Canonical serialization/ID/hash/equivalence", "AUD-01,DAT-05", "artifacts,data", "ART + DAT", "Approve representation scope", P.PARTIALLY_RESOLVED, "No unsupported lock claim", "", "T04,T36", "PARTIAL: existing typed raw/bar/manifest/lock codecs and pinned CSV golden fingerprints; operational import report not canonical persistence; representation authority approval open"),
    _ip(5, "Time/clock/timezone/ordering rules", "DAT-03,ORC-02", "DAT,STA,commands", "STA + DAT", "Approve temporal specification", P.PARTIALLY_RESOLVED, "Hold ambiguity", "", "T16,T35", "PARTIAL: strict CSV UTC timestamp and pinned availability cutoff with injected clock; vendor late-data/calendar/clock trust policy open"),
    ...
    _ip(11, "Data quality/reconciliation semantics", "DAT-01,DAT-02,DAT-03,DAT-04,DAT-05", "DAT", "DAT + GOV", "Approve source contracts", P.PARTIALLY_RESOLVED, "Quarantine unknown/conflict", "IP-04,IP-05", "T15,T16,T32", "PARTIAL: strict fixture header/decimal/geometry, raw duplicate quarantine, bar conflict/gap/incomplete findings; provider thresholds/acceptance authority open"),
    _ip(12, "Retention/privacy/licensing/legal policy", "AUD-01,AUD-02,AUD-03,DAT-01", "AUD,DAT,ledgers", "GOV + Legal + Data", "Approve policy", P.OPEN, "Preserve and restrict", "", "T25,T30", "OPEN: fixture byte hash, exact raw-to-bar-to-manifest/lock lineage and operational report improve technical custody; legal policies unresolved"),
    _ip(13, "Trust-boundary threat model/adapters", "IAM-01,HUM-01,ORC-01", "adapters,MACP,SMI", "Security + IAM", "Approve threat model", P.OPEN, "Adapters disabled", "IP-01,IP-02", "T22,T37", "OPEN: test-fixture-only path/size/encoding/header gates; arbitrary/user/network adapters unauthorized"),
    _ip(14, "Implementation governance/change/release", "GOV-01,GOV-02,AUD-03", "Phase 3", "GOV + QA + AUD", "Approve wave gates", P.PARTIALLY_RESOLVED, "No wave promotion", "", "T30,T31,T38", "PARTIAL: pinned CSV import vectors, deterministic negative tests and existing CI; release criteria open"),
)
