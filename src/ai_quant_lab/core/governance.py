"""Exact Sprint 15 implementation-precondition registry."""
...
PRECONDITIONS = (
    ...
    _ip(4, "Canonical serialization/ID/hash/equivalence", "AUD-01,DAT-05", "artifacts,data", "ART + DAT", "Approve representation scope", P.PARTIALLY_RESOLVED, "No unsupported lock claim", "", "T04,T36", "PARTIAL: governed dataset, admission, eligibility, validation and robustness artifacts persist and reconstruct with exact canonical identity; broader representation authority remains open"),
    ...
    _ip(9, "Validation method specifications", "VAL-01,VAL-02,VAL-03,VAL-04,VAL-05,VAL-06,VAL-07", "VAL", "VAL + GOV", "Approve method scopes", P.PARTIALLY_RESOLVED, "No validation claim outside exact bounded method", "IP-08", "T12,T20,T21,T22", "PARTIAL: exact fixed-strategy rolling partitions, completed-trade bootstrap and commission perturbation add deterministic robustness evidence; protected holdout evidence, dependence-aware methods, corrected multiplicity and broader method approval remain open"),
    ...
    _ip(13, "Trust-boundary threat model/adapters", "IAM-01,HUM-01,ORC-01", "adapters,MACP,SMI", "Security + IAM", "Approve threat model", P.OPEN, "Adapters disabled", "IP-01,IP-02", "T22,T37", "OPEN: eligibility binds an explicit actor ID and exact local-file provenance but does not authenticate the actor, provider or broader adapter boundary"),
    _ip(14, "Implementation governance/change/release", "GOV-01,GOV-02,AUD-03", "Phase 3", "GOV + QA + AUD", "Approve wave gates", P.PARTIALLY_RESOLVED, "No wave promotion", "", "T30,T31,T38", "PARTIAL: deterministic robustness contracts, golden evidence, verified reload, exact lineage and fail-closed tests extend CI evidence; production release criteria remain open"),
)
