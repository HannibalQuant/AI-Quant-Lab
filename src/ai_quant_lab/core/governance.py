...
PRECONDITIONS = (
    ...
    _ip(4, "Canonical serialization/ID/hash/equivalence", "AUD-01,DAT-05", "artifacts,data", "ART + DAT", "Approve representation scope", P.PARTIALLY_RESOLVED, "No unsupported lock claim", "", "T04,T36", "PARTIAL: DatasetManifest/DatasetLock/NormalizedBarManifest persist and reconstruct with exact canonical identity; broader representation authority and operational report persistence remain open"),
    _ip(5, "Time/clock/timezone/ordering rules", "DAT-03,ORC-02", "DAT,STA,commands", "STA + DAT", "Approve temporal specification", P.PARTIALLY_RESOLVED, "Hold ambiguity", "", "T16,T35", "PARTIAL: strict CSV UTC/availability rules remain; immutable store adds no storage-time semantics; vendor late-data/calendar/clock trust policy open"),
    ...
    _ip(11, "Data quality/reconciliation semantics", "DAT-01,DAT-02,DAT-03,DAT-04,DAT-05", "DAT", "DAT + GOV", "Approve source contracts", P.PARTIALLY_RESOLVED, "Quarantine unknown/conflict", "IP-04,IP-05", "T15,T16,T32", "PARTIAL: strict synthetic import quality and immutable reload preserve dispositions but storage grants no provider acceptance authority"),
    _ip(12, "Retention/privacy/licensing/legal policy", "AUD-01,AUD-02,AUD-03,DAT-01", "AUD,DAT,ledgers", "GOV + Legal + Data", "Approve policy", P.OPEN, "Preserve and restrict", "", "T25,T30", "OPEN: append-only exact-key local persistence materially advances bounded technical custody; retention/privacy/licensing/legal policy and recovery remain unresolved"),
    _ip(13, "Trust-boundary threat model/adapters", "IAM-01,HUM-01,ORC-01", "adapters,MACP,SMI", "Security + IAM", "Approve threat model", P.OPEN, "Adapters disabled", "IP-01,IP-02", "T22,T37", "OPEN: explicit allowed-root and symlink/traversal/resource gates bound the synthetic local store; arbitrary user/external data remain unauthorized"),
    _ip(14, "Implementation governance/change/release", "GOV-01,GOV-02,AUD-03", "Phase 3", "GOV + QA + AUD", "Approve wave gates", P.PARTIALLY_RESOLVED, "No wave promotion", "", "T30,T31,T38", "PARTIAL: immutable-store golden, deterministic reload, lineage and negative tests extend CI evidence; production release criteria remain open"),
)
