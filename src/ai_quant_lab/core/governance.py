...
PRECONDITIONS = (
    ...
    _ip(4, "Canonical serialization/ID/hash/equivalence", "AUD-01,DAT-05", "artifacts,data", "ART + DAT", "Approve representation scope", P.PARTIALLY_RESOLVED, "No unsupported lock claim", "", "T04,T36", "PARTIAL: typed normalized bar/schema/timeframe/manifest codecs and golden fingerprints; representation scope authority approval open"),
    _ip(5, "Time/clock/timezone/ordering rules", "DAT-03,ORC-02", "DAT,STA,commands", "STA + DAT", "Approve temporal specification", P.PARTIALLY_RESOLVED, "Hold ambiguity", "", "T16,T35", "PARTIAL: fixed UTC epoch interval/alignment, bar-close vs availability and mechanical gaps; calendar/late-data/clock trust policy open"),
    ...
    _ip(11, "Data quality/reconciliation semantics", "DAT-01,DAT-02,DAT-03,DAT-04,DAT-05", "DAT", "DAT + GOV", "Approve source contracts", P.PARTIALLY_RESOLVED, "Quarantine unknown/conflict", "IP-04,IP-05", "T15,T16,T32", "PARTIAL: synthetic OHLC geometry, finality, volume semantics, duplicate/conflict and gap findings; provider thresholds/acceptance authority open"),
    _ip(12, "Retention/privacy/licensing/legal policy", "AUD-01,AUD-02,AUD-03,DAT-01", "AUD,DAT,ledgers", "GOV + Legal + Data", "Approve policy", P.OPEN, "Preserve and restrict", "", "T25,T30", "OPEN: exact raw-to-bar-to-normalized-manifest/lock lineage strengthens technical custody; retention/privacy/licensing/legal policy unresolved"),
    ...
    _ip(14, "Implementation governance/change/release", "GOV-01,GOV-02,AUD-03", "Phase 3", "GOV + QA + AUD", "Approve wave gates", P.PARTIALLY_RESOLVED, "No wave promotion", "", "T30,T31,T38", "PARTIAL: synthetic normalized-bar codecs/golden negative tests and unchanged CI; release criteria open"),
)
